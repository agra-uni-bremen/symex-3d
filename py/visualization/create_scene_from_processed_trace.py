...
def create_control_flow_blocks(blocks_xml_root, code_string, global_start):
    print(f"{terminal_colors.BOLD}[Creating Control Flow Blocks]{terminal_colors.ENDC}")
    cf_blocks_root = blocks_xml_root[0]#TODO maybe add a check here for correct tag
    ...
    for run in cf_blocks_root:
        for block in run:
            ...
            location_t = ((block_start - global_start)*cfg.INSTRUCTION_DISTANCE+cfg.INSTRUCTION_DISTANCE, 
                            -cfg.RUN_DISTANCE-cfg.CUBE_SIZE*2, 
                            cfg.BLOCK_Z+cfg.CUBE_SIZE)
            name_t = f"cf_code_{hex(block_start)}_{hex(block_end)}"
            text = f"{file_name}\n{line_start} - {line_end}" #TODO include code
            if(bu.get_object(name_t) is None):
                text_obj = bu.create_text(location_t, name_t, text, 2, "mat_cf_block")#TODO probably better to calculate location in one place or create block text once for one run
            else:
                pass #print(f"line text object already exists for CFB {name_t}")
        run_id +=1
...
def create_scene_from_processed_trace(runs, analysis_data):
    ...
    print(f"{terminal_colors.BOLD}-----------------------------------------------------------------------------------{terminal_colors.ENDC}")
    print(f'{terminal_colors.BOLD}[Building scene from processed trace] ({time_create_start.strftime("%H:%M:%S")}):{terminal_colors.ENDC}')
    print(f"{terminal_colors.BOLD}-----------------------------------------------------------------------------------{terminal_colors.ENDC}")
    ...
    for run in runs:
        ...
        print(f"{terminal_colors.OKBLUE}RUN[{run.run_id}]{terminal_colors.ENDC} start:{run.start} parent_id:{run.parent_id} start_step:{run.start_step} start_pc:{hex(run.start_pc)}")

        cam = bd.create_camera(run_id)

        active_marker = bd.create_active_marker(f"marker_{run_id}", run_id)

        location_t = (0, run_id*cfg.RUN_DISTANCE - cfg.RUN_DISTANCE/2, 4)

        # create text object displaying parent id
        # name_t = f"text_run_of_parent {parent_id}"
        # text_t = f"child of path\n{parent_id}"
        # parent_text_obj = bu.create_text(location_t, name_t, text_t, 4, "mat_text")

        # create text object displaying run id
        name_t = f"text_run_id {run_id}"
        text_t = f"path\n{run_id}"
        parent_text_obj = bu.create_text(location_t, name_t, text_t, 3, "mat_text")

        if(len(run.instruction_list)==0):
            print(f"{terminal_colors.WARNING}[WARNING] Empty run with id [{run_id}]{terminal_colors.ENDC}")
            continue
        if(parent_id!=-1):
            #create run_start marker
            run_actual_start = run.instruction_list[0].pc
        ...
        for instruction in run.instruction_list:
            ...
            depth = instruction.depth #todo add a way to get next depth
            ...
            if(type==Opcode_type.Branch):
                ...
                for step, sym_beh in instruction.steps_active:
                    #preserve old value
                    bu.keyframe_preserve(obj,"color",0,step, offset=1)
                    bu.keyframe_preserve(obj,"color",2,step, offset=1) #TODO only relevant for symbolic jump behavior
    ...
    print(f'{terminal_colors.OKGREEN}[All runs completed]{terminal_colors.ENDC} (Duration: {time_total_time} Average: {time_average_run_duration})')
    ...
    for parent_id, run in enumerate(discovered_links):
        for child in run:
            #bu.create_text(location=(0,-10,0), name=f"dc {parent_id}: {child[0]} s {child[1]}", text=f"dc {parent_id}: {child[0]} s {child[1]}", scale=1, material="mat_text")
