...
def create_ground_material(material_name):
    """create material for PC ground plane"""
...
def create_materials():
    """create all procedural shader nodes materials"""
...
def create_ground(run:int, global_start:int, min_pc:int, max_pc:int):
    """create PC ground plane and adjust its position"""
...
def set_frame(step:int):
    """set the current Blender animation frame"""
...
def keyframe_preserve(obj,data_p,ind:int,frame_p:int, offset:int=2):
    """ Add a keyframe for the specified object data to preserve any old value that might have been set previously
...
def assign_material(obj,mat_name:str):
    """assign material with name mat_name to obj
       if mat_name doesn't exist create a placeholder material and assign that one"""
    mat = bpy.data.materials.get(mat_name)
    if mat is None:
        # create material
...
def connect_points_with_curve(curve, point_start, point_end, num_points=5):
    ...
    for i in range(1,num_points-1):
        target_point = tuple(map(sum,zip(target_point,(dist/num_points,offset,0))))
        set_curve_path_point(curve,i,target_point)

    set_curve_path_point(curve,num_points-1,point_end)
...
def create_text(location:tuple, name:str, text:str, scale:float, material):
    """create text object"""
...
def scale_object(scale:tuple,pivot=""):
...
def rotate_object(rotation:float,axis:str='Z',pivot:str=""):
...
def translate_object(translation:tuple):
