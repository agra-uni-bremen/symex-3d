...
from ptrace.Data.instructions import Link_Data
...
def parse_ptrace_xml(root):
    ...
    for run in root[0]: #in runs
        ...
        current_run.instruction_list = instruction_list
...
def parse_analysis_xml(root):
    ...
    for run in analysis[0]:
        ...
        run_start.append((r_parent, r_pc, r_step))
        
        cr_potential_child_branches = [] #[potential child branches]
        for child in run:
            link_pc = int(child.attrib.get('pc'),16)
            link_step = int(child.attrib.get('step'))
            link_hash = int(child.attrib.get('hash'))
    ...
    for run in analysis[1]:
        ...
        for child in run:
            ...
            child_pc_start = int(child.attrib.get('pc'))
            dc_run_list.append((child_id, child_pc_start, child_step_start))
